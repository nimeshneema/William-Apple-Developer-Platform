    // A C prorgam demonstrating how we can interchange the working mechanism between a counter based vs a condition based loop.
...
int main(int argc, char *argv[]) {
    char name[] = "William";
    int length_of_name = 0; // The entity length_of_name is a variable and it is possible to acidentally and inadvertantly change to an incorrect value. But C programming lanaguge does not have a device/mechanism to make constants.
    ...
    const int length_of_string = length_of_name;    // A more scalable approach. The source code adapts to any change in the contents of the `name` characer array variable.
